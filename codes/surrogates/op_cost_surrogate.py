...
parser.add_argument('--hidden_units', type=int, nargs='+', default=[64, 128, 64, 128, 64], help='List of hidden units per layer.')
parser.add_argument('--reg_coeffs', type=float, nargs='+', default=[0.05, 0.05, 0.05, 0.05, 0.05], help='List of regularization coefficients corresponding to each layer.')
...
parser.add_argument('--model_name', type=str, default='FR_op_model', help='Name to save the trained model under.')
...
y = np.array(pd.read_csv(args.output, header=None))[:, 2].reshape(-1, 1)
