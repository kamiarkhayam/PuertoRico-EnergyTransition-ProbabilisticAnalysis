...
caps, acts, emissions = read_output_fr(output_path, output_name)
caps = {key: value * 1.2 for key, value in caps.items()}
bio_max_act = caps['Bio'] * 365 * 24 / 277.78 * 0.85
...
for k in range(external_loops):
    ...
    for j in range(internal_loops):
       #Modifying Acts based on Weather
        ...
        const_acts = acts['Wind'] + acts['Sol'] + acts['Bio']
        ...
        change_in_bio_act = max(demand_2050 - acts_in['Sol'] - acts_in['Wind'], 0) / acts_in['Bio']
        acts_in['Bio'] *= change_in_bio_act
        
        if acts_in['Bio'] > bio_max_act:
            added_cost_ratio = ((acts_in['Bio'] - bio_max_act) / acts_in['Bio']) * 1.2 + (bio_max_act / acts_in['Bio']) * 1
            
        acts_in['Biofuel'] *= change_in_bio_act
        ...
        op_cost_undamaged, sol_cost, wind_cost, batt_cost, hyd_cost, bio_cost, ngcc_cost, nuc_cost, trans_cost, cond_cost = compute_normal_op_costs(
    ...
    np.savetxt(os.join.path(input_output_dir, 'outputFR.txt'), np.array(outputs))
    np.savetxt(os.join.path(input_output_dir,'inputFR.txt'), np.array(inputs))
