function  varargout = plot_output_3D_truss(ID,COORD, CONEC,U,MAGN,varargin)

%      plot_output_3D_truss(ID,COORD, CONEC,U,MAGN)
%    
...
zmin = min(COORD(:,3));
zmax = max(COORD(:,3));
% MeshMaxSize = max((xmax-xmin) , (ymax -ymin),(zmax -zmin));
set(gca,'Xlim', [xmin-0.15*(xmax-xmin) , xmax + 0.15 * (xmax-xmin)]);
set(gca,'Ylim', [ymin-0.15*(ymax-ymin) , ymax + 0.15 * (ymax-ymin)]);
set(gca,'Zlim', [zmin-0.15*(zmax-zmin) , zmax + 0.15 * (zmax-zmin)]);
% set(gca,'Xlim', [xmin-0.15*MeshMaxSize , xmax + 0.15 * MeshMaxSize]);
% set(gca,'Ylim', [ymin-0.15*MeshMaxSize , ymax + 0.15 * MeshMaxSize]);
% set(gca,'Zlim', [zmin-0.15*MeshMaxSize , zmax + 0.15 * MeshMaxSize]);
...
line(theCoords(: ,1 ) , theCoords(: ,2 ), theCoords(: ,3 ), 'color' , 'k', 'LineStyle' , '-');
end;
...
% Plot deformed shape
  DeformedShape = COORD + reshape(U, 3 , NbNodes)' * MAGN ; 
  for elnum = 1 : NbElts
	for node = 1 : NodePerElt
...
%     line(theCoords(: ,1 ) , theCoords(: ,2 ), 'color' , 'r');
	line(theCoords(: ,1 ) , theCoords(: ,2 ),theCoords(: ,3 ), 'color' , 'r','LineStyle' , ':');
  end
end
hold off;
