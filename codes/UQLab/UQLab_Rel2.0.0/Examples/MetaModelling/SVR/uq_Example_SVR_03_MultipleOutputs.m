%% SVR METAMODELING: MULTIPLE OUTPUTS
%
% This example showcases an application of Support Vector Machine for 
% Regression (SVR) to the metamodeling of a simply supported beam model
...
% where $N$ and $M$ are the numbers of realizations and input variables,
% respectively.
...
%%
% Select the L1 penalization scheme:
MetaOpts.Loss = 'l1-eps';

%%
% Use the anisotropic Gaussian kernel function:
...
%%
% Use the smoothed leave-one-out (LOO) span estimate
% to calibrate the hyperparameters:
MetaOpts.EstimMethod = 'SmoothLOO';

%%
% Use the BFGS optimization algorithm:
...
%%
% Evaluate the SVR metamodel at the same sample points:
YSVR = uq_evalModel(mySVR,Xval);
...
% is compared against the one predicted by the SVR metamodel
% $\mathcal{M}^{SVR}(\mathbf{x}^{(i)})$ (its mean):
...
uq_legend(...
    {'$\mathrm{\mathcal{M}(\mathbf{x}^{(1)})}$',...
        '$\mathrm{\mathcal{M}^{SVR}(\mathbf{x}^{(1)})}$',...
        '$\mathrm{\mathcal{M}(\mathbf{x}^{(2)})}$',...
        '$\mathrm{\mathcal{M}^{SVR}(\mathbf{x}^{(2)})}$',...
        '$\mathrm{\mathcal{M}(\mathbf{x}^{(3)})}$',...
        '$\mathrm{\mathcal{M}^{SVR}(\mathbf{x}^{(3)})}$'},...
    'Location', 'north')
