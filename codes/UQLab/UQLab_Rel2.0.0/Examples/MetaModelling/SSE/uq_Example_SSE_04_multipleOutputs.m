%% SSE METAMODELING: MULTIPLE OUTPUTS
%
% This example showcases an application of stochastic spectral embedding
% (SSE) to the metamodeling of a simply supported beam model with multiple 
% outputs. The model computes the deflections at several points along the 
% length of the beam subjected to a uniform random load.
...
%
% Select SSE as the metamodeling technique:
metaOpts.Type = 'Metamodel';
metaOpts.MetaType = 'SSE';

%%
% Specify the range of residual expansion degrees:
...
Y_SSE = uq_evalModel(mySSE, Xval);

%% 5.2 - Create plots
%
% For each sample points of the validation set $\mathbf{x}^{(i)}$,
% the simply supported beam deflection $\mathcal{M}(\mathbf{x}^{(i)})$
% is compared against the one predicted by the SSE metamodel
% $\mathrm{\mathcal{M}^{PCE}_{SSE}(\mathbf{x}^{(i)})}$:
...
uq_legend({'$\mathrm{\mathcal{M}(\mathbf{x}^{(1)})}$',...
    '$\mathrm{\mathcal{M}_{\mathrm{SSE}}(\mathbf{x}^{(1)})}$',...
    '$\mathrm{\mathcal{M}(\mathbf{x}^{(2)})}$',...
    '$\mathrm{\mathcal{M}_{\mathrm{SSE}}(\mathbf{x}^{(2)})}$',...
    '$\mathrm{\mathcal{M}(\mathbf{x}^{(3)})}$',...
    '$\mathrm{\mathcal{M}_{\mathrm{SSE}}(\mathbf{x}^{(3)})}$'},...
    'Location', 'north')