...
classdef uq_core_analysis < uq_core_module
    % the core module identifier
    properties(Constant,Hidden=true)
        core_module_identifier = 'analysis';
    end
    
    methods(Static, Hidden=true)
        function success = report()
            success = fprintf('uq_analysis status report:\n');
        end
    end
            ...
            if ~private_flag && ~overwrite_flag
                for ii = 1:length(this.module_names)
                    if strcmp(this.module_names{ii}, name)
                        error('Analysis Names must be unique! Your request to add an analysis named "%s" cannot be completed!', name) ;
                    end
                end
            end
            
            % % create a uq_analysis leaf object
            %obj = uq_analysis(name, type, varargin);
