function [success] = uq_sse_test_enrichment(level)
...
% enrichment
metaOpts.ExpDesign.Enrichment = @(obj, currBounds, nEnrich) uq_SSE_enrichment_uniform(obj, currBounds, nEnrich);
...
try
  ...
  functionHandleInfo = functions(mySSE.SSE.ExpDesign.Enrichment);
  if ~strcmpi(functionHandleInfo.function, '@(obj,currBounds,nEnrich)uq_SSE_enrichment_uniform(obj,currBounds,nEnrich)')
      error('Enrichment function handle not assigned properly')
  end
