function pass = uq_Dispatcher_tests_usage_UQLinkCommandOpts(DispatcherObj)
%UQ_DISPATCHER_TESTS_USAGE_UQLINKCOMMANDOPTS tests a dispatched computation 
%   of a UQLink model with the command line interface of the third-party
%   code having named options.
%
%   The 3rd-party command line interface is of the form:
%
%       <executable> --input <input file> --output <output file>
%
%   NOTE:
%   - This tests uses a Python3 implementation of the Ishigami function
%     named 'uq_ishigami_opts.py'; it requires numpy.
...
COMMANDLINE = sprintf('%s --input %s --output %s',...
    EXECNAME, INPUTFILE, OUTPUTFILE);
