function pass = uq_Dispatcher_tests_usage_UQLinkMultiInpOpts(DispatcherObj)
%UQ_DISPATCHER_TESTS_USAGE_UQLINKINPOUT tests a dispatched computation 
%   of a UQLink model with the command line interface of the third-party
%   code having multiple named options to specify input files.
%
%   The 3rd-party command line interface is of the form:
%
%       <executable> --input1 <inputfile1> \
%                    --input2 <inputfile2> \
%                    --input3 <inputfile3> \
%                    --output <outputfile>
%
...
%     named 'uq_ishigami_multiopts.py'; it requires numpy.
...
EXECNAME = 'uq_ishigami_multiopts.py';

% Provide the name of the input file
INPUTFILE1 = 'input_file1.csv';
INPUTFILE2 = 'input_file2.csv';
INPUTFILE3 = 'input_file3.csv';
OUTPUTFILE = 'output_file.csv';

% Create the command string to execute the 3rd-party executable
COMMANDLINE = sprintf('%s --input3 %s --input1 %s --input2 %s --output %s',...
    EXECNAME, INPUTFILE3, INPUTFILE1, INPUTFILE2, OUTPUTFILE);

% Set the command string to the UQLink MODEL options
ModelOpts.Command = COMMANDLINE;

% Specify the template file
ModelOpts.Template = {'input_file1.csv.tpl',...
    'input_file2.csv.tpl', 'input_file3.csv.tpl'};
